...
# 📄 로컬 문서 분석기 (Local Doc Analyzer)

**개인 PC에서 직접 실행되는 로컬 AI 기반 문서 요약 도구입니다.**

기존 AI 요약 서비스는 문서를 외부 서버에 업로드해야 하지만, 이 앱은 **AI가 내 컴퓨터 안에서 실행**됩니다. 현재 **PDF와 Word(.docx)**를 지원하며, HWPX·PPTX·EPUB는 지원 예정입니다.

- **완전한 오프라인 동작** — Ollama 로컬 AI 엔진이 PC에서 직접 실행되어, 문서 파일이 외부 서버로 전송되지 않습니다
...
- **페이지 인용 + 문서 뷰어** — 요약/답변에 `[p.12]` 출처 인용이 자동으로 붙어 클릭하면 원문의 해당 위치가 바로 열립니다(PDF는 페이지 렌더링, Word 문서는 텍스트 패널). 내장 목차(북마크)가 있는 PDF는 뷰어 사이드바에서 클릭해 해당 섹션으로 점프할 수도 있습니다
- **요약 마인드맵** — 어떤 요약이든 제목 구조의 인터랙티브 마인드맵으로 전환; 가지를 접기/펼치고 노드의 `[p.N]` 배지를 클릭하면 원문 뷰어의 해당 페이지로 점프
- **수식이 수식으로 보입니다** — AI 가 원문에서 옮겨 온 수식이 LaTeX 원본이 아니라 조판된 수식으로 표시됩니다. 요약·Q&A 답변·내보낸 PDF 모두
- **다중 문서 탭 + 교차 문서 Q&A** — 여러 문서를 탭으로 열고 한 번의 질문으로 함께 검색합니다. 답변은 출처 문서를 표기하고 해당 페이지로 바로 이동합니다
- **컬렉션 + 교차 문서 요약** — 함께 보는 문서 묶음을 이름과 함께 저장해 나중에 탭 세트째 다시 열고, 선택한 문서들의 통합 요약 또는 비교 분석을 생성합니다
- **세션 자동 저장·복원** — 분석한 문서를 다시 열면 요약·Q&A·검색 인덱스가 재요약·재임베딩 없이 즉시 복원됩니다
...
> **참고**: v1.8.0 이전 릴리즈는 개명 전 이름인 `Local-PDF-Analyzer-Setup-x.x.x.exe` 로 배포되었습니다. 이전 버전을 검증할 때는 그 파일명을 사용하세요.
...
> v1.8.0 이전 릴리즈를 검증할 때는 위 명령의 파일명을 `Local-PDF-Analyzer-Setup-x.x.x.exe` 로 바꿔서 사용하세요.

## 사용 방법

### 1. 문서 업로드
- 앱 화면에 PDF 또는 Word(.docx) 파일을 **드래그앤드롭**하거나, **파일 선택** 버튼 또는 **Ctrl+O**로 선택합니다
- 이전에 분석한 문서는 업로드 화면 하단 **최근 문서** 목록에서 바로 열 수 있고, 같은 문서를 다시 열면 요약·Q&A·검색 인덱스가 **자동 복원**됩니다
- **저장 문서 전체 검색** — 업로드 화면의 검색바로 저장된 모든 세션(페이지 텍스트·요약·파일명)에서 키워드를 찾습니다. 결과에 매칭 페이지가 하이라이트 발췌로 표시되며, 클릭하면 바로 열립니다
- **여러 문서를 탭으로** — 새 문서를 열면 상단에 탭이 추가되고, 탭 클릭으로 문서를 오가며 각각의 요약·Q&A를 이어서 사용할 수 있습니다 (전환 시 자동 저장·복원). `＋` 버튼으로 문서를 추가합니다

### 2. 요약 유형 선택

| 유형 | 설명 |
|------|------|
| **전체 요약** | 문서 전체 내용을 하나의 요약으로 정리 |
...
- **수식이 조판되어 표시됩니다** — 요약 지시문은 AI 에게 원문의 중요한 수식을 그대로 옮기라고 요청하는데, 그렇게 돌아온 수식이 LaTeX 원본이 아니라 실제 수식으로 보입니다. 잘못된 형태로 도착한 수식은 그 조각만 원문 그대로 표시되고 요약의 나머지는 서식을 잃지 않습니다. 이 기능이 생기기 전에 저장한 요약도 다음에 열 때 함께 조판됩니다
- **`.md` 내보내기**·**PDF 내보내기**(헤딩/표/인용이 담긴 서식 PDF)로 파일 저장, **복사** 버튼으로 클립보드에 복사
- 요약 닫기는 비파괴적 — 문서 화면으로 접히고, **요약 보기 / Q&A 계속**으로 Q&A 스레드까지 그대로 다시 엽니다

### 4. Q&A 채팅 (RAG 시맨틱 검색)
- 문서 로드 시 자동으로 **RAG 벡터 인덱스**가 생성됩니다 (헤더에 진행률 → 완료 시 **RAG** 배지 표시). 앱을 켜둔 채 Ollama 임베딩 모델을 설치·삭제해 인덱스가 현재 임베딩 모델과 맞지 않게 되면, 초록 배지 뒤에서 조용히 키워드 검색으로 내려가지 않고 헤더에 그 사실을 표시합니다
- 질문하면 임베딩 유사도로 문서에서 가장 관련 높은 부분을 찾아 AI가 답변합니다 (최대 10턴까지 이전 대화 맥락 유지)
- 임베딩 모델이 없으면 키워드 기반 검색으로 자동 전환됩니다 (기능 동일, 정확도 차이)
- **답변 자동 검증** — 답변의 각 문장을 문서 임베딩에 대조하여, 근거가 약한 문장이 많으면 자동으로 한 번 더 다듬어 출력합니다 (설정에서 비활성화 가능)
...
- **요약 ↔ 채팅 높이 조절** — 요약 본문과 Q&A 채팅 사이의 구분선을 드래그하거나 키보드(Tab 포커스 후 `↑`/`↓`, `Home`/`End`)로 위아래 비율을 20~80% 사이에서 조정합니다. 비율은 저장되어 재시작 시 복원됩니다 — 컬렉션 통합 요약·비교 분석의 결과가 채팅 쪽에 표시되므로, 긴 결과를 볼 때 채팅 영역을 넓히면 편합니다
- `Enter`: 전송 / `Shift+Enter`: 줄바꿈

### 5. 페이지 인용 + 문서 뷰어
- 요약과 Q&A 답변의 핵심 사실마다 **`[p.12]` 형태의 페이지 인용**이 자동으로 붙습니다
- 인용을 **클릭**하면 화면 우측에 **문서 뷰어 패널**이 열려 해당 위치로 바로 이동합니다 — PDF는 실제 페이지가 렌더링되고, Word 문서는 해당 구간으로 스크롤된 텍스트 패널이 열립니다. AI 환각 여부를 1-click으로 검증할 수 있습니다
- **목차 네비게이션 (PDF 전용)** — PDF에 내장 목차(북마크)가 있으면 뷰어의 ☰ 버튼으로 목차 사이드바가 열리고, 항목을 클릭하면 해당 페이지로 이동합니다(현재 섹션을 다시 클릭하면 재스크롤)
- **확대·축소** — 뷰어 상단의 `−` / `+` 버튼(25% 단위), `Ctrl`+마우스 휠(10% 단위), 또는 뷰어가 열린 상태에서 `Ctrl`+`=` / `Ctrl`+`-` / `Ctrl`+`0`. 50~300% 범위이며 퍼센트 표시를 클릭(또는 `Ctrl`+`0`)하면 화면 맞춤으로 돌아갑니다(도면처럼 아주 큰 페이지는 메모리 보호를 위해 상한이 300%보다 낮아질 수 있고, 그때 표시되는 퍼센트는 실제로 그려진 배율입니다). 배율을 바꿔도 보던 위치가 유지되고, 배율은 재시작 후에도 기억됩니다 — 작은 글씨 논문이나 스캔 문서에 유용합니다
...
PDF와 Word 문서에 포함된 차트, 다이어그램, 표, 사진 등을 Vision AI가 자동으로 분석하여 요약에 포함합니다.

- 문서에서 이미지를 개별 추출하여 Vision 모델로 의미 분석
- 분석 결과가 해당 본문 텍스트에 자연스럽게 통합되어 요약 품질 향상
...
- 인식에 실패한 페이지가 지나치게 많으면 결과를 저장하지 않고 실패로 끝냅니다 — 도중에 인식이 멈춘 문서가 "완료"로 저장되어, 이전에 잘 인식해 둔 같은 문서의 결과를 덮어쓰지 않도록 합니다
...
**분석 품질**
- 한국어 최적화 — 한글 PDF 텍스트 추출 품질 개선, 한글 비율에 따른 청크 자동 조절(지원하는 모든 포맷 공통)
- 깔끔한 요약 — AI가 생성하는 인사말·감상평·대화형 멘트를 프롬프트 제약 + 후처리 필터로 이중 제거
- 대용량 문서 지원 — 긴 문서도 자동 분할 후 배치 병렬 처리, 통합 요약 생성 (최대 500페이지)
- 답변 자동 검증 — Q&A 답변을 문장 단위로 문서 임베딩에 대조, 근거 약한 답변은 자동 재정리
...
- 다중 문서 탭 — 여러 문서를 열어두고 전환, 무거운 상태는 활성 문서만 메모리에 유지(전환 시 세션 복원으로 즉시). 컬렉션 모드로 열린 문서들에 걸쳐 한 번에 질문하고 출처를 표기, 묶음을 이름으로 저장하고 통합/비교 요약 생성
- PDF 목차 네비게이션 — 내장 목차가 있는 PDF 문서는 뷰어에 접이식 목차 사이드바가 표시되어 원클릭으로 원하는 섹션으로 점프
- 뷰어 확대·축소 — 상단 버튼·`Ctrl`+휠·`Ctrl`+`=`/`-`/`0` 으로 50~300%, 배율을 바꿔도 보던 위치 유지, 배율은 재시작 후에도 기억
- 요약 마인드맵 — 텍스트/마인드맵 토글로 요약의 제목 계층을 접을 수 있는 트리로 렌더; 각 노드에 페이지 인용이 `[p.N]` 배지로 붙어 원클릭으로 원문 페이지로 점프(키보드·스크린리더 친화, 외부 의존성 추가 없음)
- 수식 표기 — 요약·Q&A 답변·내보낸 PDF 에서 수식을 조판해 표시. 브라우저 엔진에 내장된 수식 레이아웃을 쓰므로 웹폰트를 내려받지 않고, 스크린리더가 수식의 원본이 아니라 구조를 읽습니다. `$100` 같은 금액 표기를 수식으로 오인하지 않습니다
...
- 끊긴 요약 회수 — 모델이 내려가거나 연결이 끊겨 생성이 중단되면, 그때까지 받은 본문을 "부분 저장"으로 남길 수 있습니다. 자동으로 저장하지는 않습니다 — 완성본과 구분되도록 미완성 표식과 함께, 사용자가 누를 때만 저장합니다
- 자동 업데이트 — 시작 시 새 버전을 감지해 클릭 한 번으로 설치. 동의 없이 다운로드하지 않으며, 재시작 전에 작업 중이던 내용을 저장합니다

**품질 보증**
- 단위 테스트 2567건 + Playwright E2E + CI 품질 게이트, 릴리즈마다 4-에이전트 병렬 QA 수행
...
- 문서 제한: 최대 100MB, 최대 500페이지 (초과 시 문서 분할 권장)
...
| 요약이 문장 중간에서 끝남 | 모델이 출력 한도에 도달한 경우입니다. 그럴 때는 요약 말미에 그 사실이 표시되므로, 잘린 요약이 완성본처럼 저장되지 않습니다. 설정에서 청크 크기를 줄이거나 출력 한도가 큰 모델을 쓰세요 |
| Ollama 요약이 HTTP 숫자만 남기고 실패 | 이제 실제 사유가 표시됩니다 — 설정에 지정된 모델이 더 이상 설치돼 있지 않거나(`ollama pull <모델>`, 또는 설정 → 모델 관리에서 다른 모델 선택), 모델을 올릴 메모리가 부족한 경우(더 작은 모델을 쓰거나 다른 프로그램을 종료)입니다 |
...
| OCR이 너무 오래 걸림 | 진행 중 "■ 취소" 버튼으로 중단할 수 있습니다. 클라우드 provider로 전환하면 더 빠릅니다. Vision 모델이 응답을 멈춘 경우에는 앱이 스스로 포기하므로, 남은 페이지를 끝까지 기다리지 않습니다 |
| 문서가 500페이지 초과 | 수동으로 문서를 분할한 후 다시 업로드해주세요. 자원 폭주 방지를 위해 상한이 적용됩니다 |
| 이미지 분석이 안 됨 | Ollama 사용 시 llava 등 Vision 모델이 필요합니다. 설정에서 모델을 설치해주세요 |
| 이미지 분석이 건너뛰지 않고 실패로 표시됨 | Vision 응답이 비어 있거나 프로바이더 안전 필터에 걸린 경우를 이제 알립니다 — 이전에는 그런 이미지가 요약에서 조용히 빠졌고, 그럼에도 요금은 이미 청구된 상태였습니다. 다른 Vision 모델로 바꾸거나 설정에서 이미지 분석을 끄세요 |
...
| 설정에 "키 확인 불가"가 표시됨 | 그 순간 키 파일을 읽지 못한 상태입니다 — 보통 백신이나 검색 인덱서가 파일을 잠깐 잡고 있는 경우입니다. 키가 사라진 것이 아니므로 잠시 후 설정을 다시 열면 됩니다. 잠긴 파일이 "키 미저장"으로 보이지 않도록 일부러 구분해 표시합니다 |
| Gemini에서 "응답이 차단되었습니다" 에러 | Gemini 안전 필터가 문서 내용을 차단했거나 출력 한도를 초과한 경우입니다. 다른 모델(gemini-2.5-pro 등)로 바꾸거나 문서를 분할해보세요 |
| Gemini "요청 한도를 초과했습니다 (rate limit)" | 무료 티어는 분당 요청 수 제한이 낮습니다. 앱이 자동으로 동시 요청을 줄이고 최대 2회 재시도하지만, 계속 발생하면 잠시 후 다시 시도하거나 이미지가 많은 문서는 이미지 분석을 끄고 사용해보세요 |
...
| 의미 검색에서 일부 문서가 제외됐다고 표시됨 | 그 문서들의 검색 인덱스가 손상되었거나 없는 상태입니다(저장이 중간에 끊겼거나 앱 밖에서 파일이 지워진 경우). 이전에는 아무 안내 없이 결과에서만 빠져 "관련 내용이 없다"로 읽혔습니다 — 해당 문서를 다시 열면 인덱스가 재생성됩니다 |
...
| 수식이 이상하게 보임 | AI 가 추출된 텍스트에서 수식을 옮겨 적기 때문에 잘못 옮길 수 있습니다 — 옆의 `[p.N]` 인용을 눌러 원문 페이지와 대조하세요. 더 강한 모델(또는 클라우드 프로바이더)이 수식을 더 정확히 옮깁니다. 빨간색으로 표시된 조각은 AI 가 수식으로 읽을 수 없는 형태로 만들어 낸 부분입니다 |
| 내보낸 PDF 에서 아주 넓은 수식이 잘림 | 인쇄 페이지는 가로로 스크롤되지 않고 수식은 줄바꿈되지 않아, 페이지보다 넓은 수식은 오른쪽이 잘립니다. 블록 수식은 이미 축소하고 페이지 여백까지 끌어써서 최대한 담고 있습니다 — 그보다 넓은 수식은 화면에서 보거나(가로 스크롤됨) `[p.N]` 인용으로 원문 페이지를 확인하세요 |
...
| 테스트 | Vitest 단위 테스트 2567건/122파일 (renderer·shared 1675 + main 892) + Playwright E2E (CI-결정적 10건) + `tsc --noEmit` 타입 체크 + CI 커버리지 게이트 (82/75/81/85) |
...
- **단위 테스트 2567건 / 122파일** — renderer·shared 1675 + main 892. 메인 프로세스는 electron 모킹 하니스로 IPC 핸들러·OllamaManager·API 키 저장소·ai-service·전체 문서 검색까지 행위 테스트, 렌더러/preload 레이어(컴포넌트 18종 전수 + 앱 셸 자체 + use-summarize/use-session/pdf-parser/safe-markdown 등 핵심 라이브러리 + preload 브리지)는 happy-dom 으로 행위 테스트
...
- **CI 게이트** — `tsc --noEmit`(strict, e2e 전용 타입체크 프로젝트 포함), 커버리지 임계(82/75/81/85) 강제, lockfile 버전 동기화 검증, 태그 ↔ `package.json` 버전 일치 검증, 차단성 `npm audit` 게이트 2종(프로덕션 트리 전량 + 실제 배포되는 것의 **전이 의존**까지 lockfile 폐포로 검사), 수식 청크가 초기 번들로 새지 않는지 보는 빌드 시점 검사, Node 22/24 매트릭스 + Windows 유닛 테스트 레그
