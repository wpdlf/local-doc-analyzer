...
- **확대·축소** — 뷰어 상단의 `−` / `+` 버튼(25% 단위), `Ctrl`+마우스 휠(10% 단위), 또는 뷰어가 열린 상태에서 `Ctrl`+`=` / `Ctrl`+`-` / `Ctrl`+`0`. 50~300% 범위이며 퍼센트 표시를 클릭(또는 `Ctrl`+`0`)하면 화면 맞춤으로 돌아갑니다. 배율을 바꿔도 보던 위치가 유지되고, 배율은 재시작 후에도 기억됩니다 — 작은 글씨 논문이나 스캔 문서에 유용합니다
