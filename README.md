...
| A very wide formula is cut off in the exported PDF | Print pages do not scroll sideways and mathematical notation does not wrap, so a formula wider than the page loses its right edge. Block formulas are already shrunk and pulled into the page margins to fit as much as possible; for the widest ones, view the summary on screen (where it scrolls) or check the source page via the `[p.N]` citation |
