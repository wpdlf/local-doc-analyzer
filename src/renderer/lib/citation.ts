 ...
 * 인식하되 quote 내용은 무시한다. `\s*\|\s*[^\]\n]{0,200}` 는 선택적으로 소비만 한다.
 */
 ...
 * 길이를 110 으로 제한한다(뒤에 붙는 ` p.N` 여유). 정규화하지 않으면 `[2024] Report.pdf` 같은
 * 파일명이 doc 그룹에 걸리지 않아 인용이 비클릭 plain text 로 강등된다(QA9 B-LOW).
 *
 * QA21(D-MED): **생산자와 소비자가 같은 함수를 써야 한다.** 이전에는 라벨 생산(use-qa)에만
 * 이 정규화가 있고, 클릭 해석(CitationButton)은 **가공 전 원본 fileName 과 정확 일치**를
 * 요구했다. 그래서 파일명에 `[ ] |`·연속 공백이 있거나 110자를 넘으면 두 값이 구조적으로
 * 어긋나 그 문서로의 인용이 전부 "문서 닫힘" 으로 비활성화됐다(열려 있는데도).
 * 순수 함수로 분리해 양쪽이 공유한다 — citation.test.ts 가 왕복 불변식을 가드한다.
 */
export function sanitizeDocLabelName(fileName: string): string {
  return fileName
    .replace(/[[\]|\n]/g, ' ')
    .replace(/\s+/g, ' ')
    .trim()
...
export function normalizeCitationPlacement(text: string): string {
  ...
  // 보안(ReDoS): 기존 단일 정규식 `(?:[인용]\s*[.,]?\s*)+$` 은 중첩 반복 + 인접 `\s*` 로
  // 지수적 백트래킹(악성 입력 한 줄로 UI 동결)을 일으켰다. 인용 토큰을 제거한 뒤 남는 것이
  // 구분자(공백/마침표/쉼표)뿐인지 선형 검사로 대체 — CITATION_REGEX 단일 출처 재사용.
  const isStandaloneCitationLine = (line: string): boolean => {
  ...
  for (const line of lines) {
    if (isStandaloneCitationLine(line)) {
      // 이 라인의 인용들을 추출 + 이전 비어있지 않은 라인 끝에 부착 (CITATION_REGEX 단일 출처)
      const citations = Array.from(
        line.matchAll(new RegExp(CITATION_REGEX.source, CITATION_REGEX.flags)),
      ).map((m) => m[0]);
